...
- 注意事项
  ...
  - Gemini 暂不支持流式
  - 超时：非流式 `LLM_TIMEOUT_MS`（总时长，默认 60000，对齐平台时长上限）；流式 `LLM_IDLE_TIMEOUT_MS`（空闲超时，默认 60000，每收到一个 chunk 重置计时，静默超限才中止，不掐长对话）
  - 客户端断开时自动中止上游连接（AbortController）

- 自检（本地假上游 + 断言，不联网）
  - 客户端层：`npx ts-node scripts/llm-proxy-selfcheck.ts`
  - HTTP 层（起真实服务 + 假上游）：`npx ts-node scripts/llm-proxy-e2e.ts`
  - 覆盖：上游 4xx 不挂起、非流式总时长超时、流式空闲超时、流式正常透传